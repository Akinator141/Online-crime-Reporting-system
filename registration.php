...
if(isset($_POST['s'])){
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $sql = "SELECT * FROM user WHERE a_no = '$a_no'";
        $result = mysqli_query($con, $sql);
        $num_rows = mysqli_num_rows($result);
        if($num_rows>0)
        {
            echo '<script>alert("This Aadhar No. is Already Exists");</script>';
            echo "<script> window.location.href='registration.php';</script>";
        }
        ...
        if($num_rows>0)
        {
          echo '<script>alert("This Mobile No. is Already Exists");</script>';
          echo "<script> window.location.href='registration.php';</script>";
        }
       // $password=md5($u_pass);
          ...
          else
          {
            ...
            echo "<script type='text/javascript'>alert('$message');</script>";
              ...
              $mail->Username = 'abhishekkshirsagar1920@gmail.com'; // your email id
              $mail->Password = 'tieqpsqdihniajip'; // your password
              $mail->SMTPSecure = 'tls';                  
              $mail->Port = 587;     //587 is used for Outgoing Mail (SMTP) Server.
              $mail->setFrom('abhishekkshirsagar1920@gmail.com', 'Online Crime Reporting System');
              $mail->addAddress($u_id);   // Add a recipient
              $mail->isHTML(true);  // Set email format to HTML

              $bodyContent = '<h1>Dear User</h1>';

              $bodyContent .= '<p>Your Account has been registered successfully...</p>';
              $mail->Subject = 'Online Crime Reporting System - Your Account HGas Been Registered';
		 ...
		 <div class="bg-agile">
			...
			<div class="login-form">	
				<form action="#" method="post">
					...
					<a href="mailer.php"><input type="submit" value="Submit" name="s"></a>
				</form>	
			</div>	
		</div>
	</div>	
